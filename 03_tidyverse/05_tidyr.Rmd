---
title: "Stat 480 - reshaping data with tidyr"
...
## Example from the homework (2)

Cleaning the data

```{r}
ranks <- starwars %>% filter(`Have you seen any of the 6 films in the Star Wars franchise? Response`=="Yes") %>% 
  gather("episode", "rank", 10:15) %>%
  mutate(
    rank = as.numeric(rank),
    episode = gsub(".*(Episode [IV]*) .*", "\\1", episode)
  ) 
ranks %>% select(episode, rank)
```

## Example from the homework (3)

Visualizing the result

```{r}
ranks %>% group_by(episode, `Gender Response`) %>%
  summarise(
    n = n(),
    mean = mean(rank, na.rm=TRUE)
  ) %>%
  ggplot(aes(x = episode, y=mean, colour=`Gender Response`)) +
...
- `spread` is the inverse operation of `gather`
