...
Important: do not use `$` notation for variables within the function 
...
`desc` stands for descending, otherwise rows are sorted smalles to largest
...
Select specific variables of a data frame

```{r}
fbi %>% arrange(desc(Year), Type, desc(Count)) %>%
  select(State, Year, Count, Type)
```
